...
def orbmrun(
    ...
    def DragAccel(R, SMA, density_file, year):
        ALT = R - (EARTHRADEQT * 1000)  # Altitude (m)
        A2M = sc_area_d / sc_mass  # Area to mass ratio
        VEL = Velocity(R, SMA)  # Velocity magnitude (m/s)
        density_df = pd.read_csv(f"{density_file}")
        ATM = np.interp(ALT / 1000, density_df["Altitude(km)"], density_df[str(year)])
    ...
    def sun_moonAccel(date, cart_state):
        ...
        moon_pos = (
            np.array(sun_moon_pos.approxECIMoonPosition(date)) * 1e3
        )  # Moon position
    ...
    def DecayRate(R, SMA, datetime, density_file):
        DragAcceleration = DragAccel(R, SMA, density_file, datetime.year)
        SRP_Accel = SRPAccel(r_factor)
        SM_Accel = sun_moonAccel(datetime, cart_state)
        return (
            -1 * (DragAcceleration + SRP_Accel + SM_Accel) * OrbitPeriod(SMA) / math.pi
        )
    ...
    while lifetime_flag == False and total_duration < 315360000:
        ...
        decay_rate = DecayRate(rd, smaxis, tstart_dt, density_file)
    ...
    while tstart_dt <= tfinal_dt:
        ...
        if smaxis < ((orb_a - maintenance_tolerance) * 1000):
            DV, time_elapsed = HohmannTransferDV(
                rd, maintenance_tolerance, maintenance_fro
            )
            thrustcount += 1
    ...
    print(f"Final drag value: {final_drag:.3f} mN")
    ...
    return sat_epochs, sat_altitude, sat_mean_sma, total_DV, total_impulse, final_drag
